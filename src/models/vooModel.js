...
function buscarDestinos() {
    ...
    var instrucaoSql = `
        SELECT 
            estado as nome,
            SUM(CASE WHEN ano = YEAR(CURRENT_DATE())-4 THEN numEmbarques ELSE 0 END) as embarqueAtual,
            SUM(CASE WHEN ano = YEAR(CURRENT_DATE())-4 THEN numDesembarques ELSE 0 END) as desembarqueAtual,
            
            SUM(CASE WHEN ano = YEAR(CURRENT_DATE())-5 THEN numEmbarques ELSE 0 END) as embarqueAnterior,
            SUM(CASE WHEN ano = YEAR(CURRENT_DATE())-5 THEN numDesembarques ELSE 0 END) as desembarqueAnterior
        FROM voo
        GROUP BY estado
